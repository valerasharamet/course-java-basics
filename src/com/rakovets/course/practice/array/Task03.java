...
public class Task03 extends StandardInputTask {
	...
	static double[] getAverageMark(int[][] marks) {
		...
		return Arrays.stream(marks)
				...
				.mapToDouble(x -> Double.parseDouble(String.format("%.2f", x)))
				.toArray();
	}
	...
	static int[] getMinMark(int[][] marks) {
		...
		// Код, решающий задачу пишем ниже, при этом используя параметры метода
		// Для проверки решения необходимо запустить @Test для данного class (в директории test)
		return Arrays.stream(marks).map(x -> Arrays.stream(x).min()).mapToInt(x -> x.getAsInt()).toArray();
	}
	...
	static int[] getMaxMark(int[][] marks) {
		...
		// Код, решающий задачу пишем ниже, при этом используя параметры метода
		// Для проверки решения необходимо запустить @Test для данного class (в директории test)
		return Arrays.stream(marks).map(x -> Arrays.stream(x).max()).mapToInt(x -> x.getAsInt()).toArray();
	}
